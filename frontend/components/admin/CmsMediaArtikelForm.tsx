...
import {
  createMediaArtikel,
  updateMediaArtikel,
  getMediaArtikelDetail,
} from "@/lib/api";

interface CmsMediaArtikelFormProps {
  mode?: "create" | "edit";
  editSlug?: string;
}

export function CmsMediaArtikelForm({
  mode = "create",
  editSlug,
}: CmsMediaArtikelFormProps) {
  ...
  useEffect(() => {
    if (mode === "edit" && editSlug) {
      getMediaArtikelDetail(editSlug)
        .then((data) => {
          setItemId(data.id);
          setJudul(data.judul ?? "");
          setSlug(data.slug ?? "");
  ...
  const handleSubmit = async (isPublished: boolean) => {
    ...
    try {
      ...
      if (mode === "edit") {
        await updateMediaArtikel(itemId, payload);
      } else {
        await createMediaArtikel(payload);
      }
    ...
    <div className="min-h-screen bg-[#F5F7FA] font-body">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-sm text-[#6B7A99]">Artikel</span>
          <ChevronRight className="w-4 h-4 text-[#9AA3B5]" />
        ...
        <div className="mb-8">
          ...
          <p className="text-base text-[#6B7A99]">
            Buat artikel atau konten media baru
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl p-6 shadow-sm">
                ...
                <input
                  ...
                  placeholder="Teologi, Biblika, Pelayanan, dll."
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              ...
              <div className="p-6">
                <div className="rounded-lg border border-[#E8ECF2] transition-colors focus-within:border-sttb-navy">
                  <EditorContent
                    editor={editor}
                    className="[&_.ProseMirror]:min-h-[320px] [&_.ProseMirror]:px-4 [&_.ProseMirror]:py-3 [&_.ProseMirror]:outline-none [&_.ProseMirror_p]:my-2 [&_.ProseMirror_h1]:text-2xl [&_.ProseMirror_h1]:font-bold [&_.ProseMirror_h1]:my-3 [&_.ProseMirror_h2]:text-xl [&_.ProseMirror_h2]:font-bold [&_.ProseMirror_h2]:my-2 [&_.ProseMirror_ul]:list-disc [&_.ProseMirror_ul]:ml-6 [&_.ProseMirror_ol]:list-decimal [&_.ProseMirror_ol]:ml-6"
                  />
                </div>
              </div>
            </div>
